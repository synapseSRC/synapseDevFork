...
This project is a multi-module Gradle project designed for Kotlin Multiplatform (KMP).
...
- **Structure:**
    ...
    - `androidMain/`: Android-specific implementations (e.g., UUID, formatting).
...
- **Build Logic:** Dependencies are managed in `build.gradle` (root and module-level).
- **Version Catalog:** (Future) We plan to migrate to `libs.versions.toml`. currently using standard Gradle definitions.
