...
class ProfileRepositoryImpl(private val client: SupabaseClientType) : ProfileRepository {
    ...
    override fun getProfile(userId: String): Flow<Result<UserProfile>> = flow {
        ...
        try {
            android.util.Napier.d("ProfileRepository", "Loading profile for userId: $actualUserId")
            ...
            android.util.Napier.d("ProfileRepository", "Profile query response: $response")

            if (response == null) {
                android.util.Napier.e("ProfileRepository", "Profile not found for userId: $actualUserId")


                try {
                    val currentUser = client.auth.currentUserOrNull()
                    if (currentUser != null && currentUser.id == actualUserId) {
                        android.util.Napier.d("ProfileRepository", "Attempting to create missing profile for current user")
                        ...
                        android.util.Napier.d("ProfileRepository", "Created missing profile, retrying query")
                        ...
                        if (retryResponse != null) {
                            ...
                            android.util.Napier.d("ProfileRepository", "Profile created and loaded successfully")
                            emit(Result.success(profile))
                            return@flow
                        }
                    }
                } catch (createError: Exception) {
                    android.util.Napier.e("ProfileRepository", "Failed to create missing profile", createError)
                }
            ...
            NetworkOptimizer.cache(cacheKey, profile)
            android.util.Napier.d("ProfileRepository", "Profile loaded successfully for userId: $actualUserId")
            emit(Result.success(profile))
        } catch (e: Exception) {
            android.util.Napier.e("ProfileRepository", "Failed to load profile for userId: $actualUserId", e)
