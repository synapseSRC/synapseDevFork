...
object CloudinaryConfig {
    ...
    fun buildImageUrl(publicId: String?, transformation: String?): String {
        if (publicId.isNullOrEmpty()) return ""
        
        return if (transformation.isNullOrEmpty()) {
            "$BASE_URL$publicId"
        } else {
    ...
    fun buildVideoUrl(publicId: String?, transformation: String?): String {
        ...
        return if (transformation.isNullOrEmpty()) {
            "$VIDEO_BASE_URL$publicId"
        } else {
            "$VIDEO_BASE_URL$transformation/$publicId"
        }
    }

    fun buildGalleryThumbnailUrl(publicId: String?): String {
        return buildImageUrl(publicId, ImageTransformations.GALLERY_THUMBNAIL)
    }

    fun buildCarouselImageUrl(publicId: String?, size: String = "medium"): String {
        val transformation = when (size) {
            "small" -> ImageTransformations.CAROUSEL_IMAGE_SMALL
            "large" -> ImageTransformations.CAROUSEL_IMAGE_LARGE
            else -> ImageTransformations.CAROUSEL_IMAGE_MEDIUM
        }
        return buildImageUrl(publicId, transformation)
    }

    fun buildOptimizedImageUrl(publicId: String?): String {
        return buildImageUrl(publicId, ImageTransformations.FULL_RESOLUTION)
    }
