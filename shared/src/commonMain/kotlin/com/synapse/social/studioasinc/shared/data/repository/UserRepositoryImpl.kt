...
class UserRepositoryImpl(
    ...
    override suspend fun getUserById(userId: String): Result<User?> {
        return try {
            ...
            return SupabaseErrorHandler.toResult(e, TAG, "Failed to fetch user by ID: $userId")
        }
    }
    ...
    override suspend fun getUserByUsername(username: String): Result<UserProfile?> {
        return try {
            ...
            return SupabaseErrorHandler.toResult(e, TAG, "Failed to fetch user by username: $username")
        }
    }
    ...
    override suspend fun updateUser(user: UserProfile): Result<UserProfile> {
        return try {
            ...
            Napier.d("User updated successfully: ${user.uid}", tag = TAG)
            Result.success(user)
        } catch (e: Exception) {
            return SupabaseErrorHandler.toResult(e, TAG, "Failed to update user: ${user.uid}")
        }
    }

    override suspend fun searchUsers(query: String, limit: Int = 20): Result<List<UserProfile>> {
        return try {
            if (query.isBlank()) {
            ...
            Napier.d("Search found ${users.size} users for query: $query", tag = TAG)
            Result.success(users)
        } catch (e: Exception) {
            return SupabaseErrorHandler.toResult(e, TAG, "Failed to search users with query: $query")
        }
    }
    ...
    override suspend fun checkUsernameAvailability(username: String): Result<Boolean> {
        return try {
            ...
            return SupabaseErrorHandler.toResult(e, TAG, "Failed to check username availability: $username")
        }
    }
