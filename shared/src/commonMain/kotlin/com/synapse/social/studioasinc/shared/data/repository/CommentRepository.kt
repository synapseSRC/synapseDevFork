...
class CommentRepository(
    ...
    suspend fun fetchComments(postId: String, limit: Int = 50, offset: Int = 0): Result<List<CommentWithUser>> = withContext(Dispatchers.IO) {
        try {
            Napier.d("Fetching comments for post: $postId", tag = TAG)
            ...
            Napier.e("Failed to fetch comments: ${e.message}", e, tag = TAG)
    ...
    suspend fun getReplies(commentId: String): Result<List<CommentWithUser>> = withContext(Dispatchers.IO) {
        try {
            Napier.d("Fetching replies for comment: $commentId", tag = TAG)
            val response = client.from("comments")
                .select(
                    columns = Columns.raw("""
                        *,
            ...
            Napier.d("Raw response size: ${response.size}", tag = TAG)
            ...
            Napier.d("Successfully parsed ${populatedReplies.size} replies", tag = TAG)
            ...
            Napier.e("Failed to fetch paged comments: ${e.message}", e, tag = TAG)
            Result.failure(Exception(mapSupabaseError(e)))
        }
    }
    ...
    suspend fun addComment(postId: String, content: String, parentCommentId: String? = null): Result<CommentWithUser> = withContext(Dispatchers.IO) {
        try {
            ...
            val clientGeneratedId = randomUUID()
            Napier.d("Creating comment for post: $postId by user: $userId with client-generated ID: $clientGeneratedId", tag = TAG)
            ...
            repeat(MAX_RETRIES) { attempt ->
                ...
                try {
                    ...
                    Napier.d("Comment created successfully: ${comment.id}", tag = TAG)
                } catch (e: Exception) {
                    lastException = e
            ...
            Napier.e("Failed to add comment: ${e.message}", e, tag = TAG)
            Result.failure(Exception(mapSupabaseError(e)))
        }
    }
    ...
    suspend fun deleteComment(commentId: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            Napier.d("Comment deleted successfully: $commentId", tag = TAG)
            Result.success(Unit)
        } catch (e: Exception) {
            Napier.e("Failed to delete comment: ${e.message}", e, tag = TAG)
            Result.failure(e)
        }
    }
    ...
    suspend fun updateComment(commentId: String, newContent: String): Result<CommentWithUser> = withContext(Dispatchers.IO) {
        try {
            ...
            Napier.e("Failed to update comment: ${e.message}", e, tag = TAG)
            Result.failure(e)
        }
    }
    ...
    suspend fun pinComment(commentId: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            Napier.e("Failed to pin comment: ${e.message}", e, tag = TAG)
            Result.failure(e)
        }
    }
    ...
    suspend fun hideComment(commentId: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            Napier.e("Failed to hide comment: ${e.message}", e, tag = TAG)
            Result.failure(e)
        }
    }
    ...
    suspend fun reportComment(commentId: String, reason: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            Napier.e("Failed to report comment: ${e.message}", e, tag = TAG)
            Result.failure(e)
        }
    }
    ...
    private suspend fun parseCommentFromJson(data: JsonObject): CommentWithUser? {
        return try {
            ...
            Napier.e("Failed to parse comment: ${e.message}", tag = TAG)
            null
        }
    }
    ...
    private fun parseUserProfileFromJson(userData: JsonObject?): UserProfile? {
        ...
        return try {
            ...
            Napier.e("Failed to parse user profile: ${e.message}", tag = TAG)
            null
        }
    }
    ...
    private suspend fun updateRepliesCount(commentId: String, delta: Int) {
        try {
            ...
            Napier.e("Failed to update replies count: ${e.message}", tag = TAG)
        }
    }
    ...
    private suspend fun updatePostCommentsCount(postId: String, delta: Int) {
        try {
            ...
            Napier.e("Failed to update post comments count: ${e.message}", tag = TAG)
        }
    }
